...
from kaldi_preprocessing import kaldi_fold, kaldi_vocabulary, kaldi_groundtruth, kaldi_confmat

# Utility function for training, validating, and testing a model and saving the logs in a CSV file
def train_and_test_model(omr_data, amt_data, gt_data, vocabularies, epochs, model, prediction_model, pred_model_filepath, log_path):
    omr_train_images, omr_val_images, omr_test_images = omr_data
    amt_train_images, amt_val_images, amt_test_images = amt_data
    train_labels, val_labels, test_labels = gt_data
    ...
    for epoch in range(epochs):
        ...
        print("Training:")
        history = model.fit(
            train_data_generator(omr_train_images, amt_train_images, train_labels, w2i),
            epochs=1, 
            verbose=2,
            steps_per_epoch=len(omr_train_images) // config.batch_size
        )
        loss_acc.extend(history.history["loss"])
        print("Validating:")
        val_symer, val_seqer = evaluate_model(prediction_model, omr_val_images, amt_val_images, val_labels, i2w)[0:2]
    ...
    test_symer, test_seqer, test_data = evaluate_model(prediction_model, omr_test_images, amt_test_images, test_labels, i2w)
...
def k_fold_experiment(epochs, num_train_samples, num_val_samples):
    ...
    omr_train_images_fnames, amt_train_images_fnames, train_labels_fnames = get_datafolds_filenames(train_folds_files)
    omr_val_images_fnames, amt_val_images_fnames, val_labels_fnames = get_datafolds_filenames(val_folds_files)
    omr_test_images_fnames, amt_test_images_fnames, test_labels_fnames = get_datafolds_filenames(test_folds_files)

    # ---------- K-FOLD EVALUATION

    # Start the k-fold evaluation scheme
    # k = len(omr_train_images_fnames)
    # for i in range(k):
    # We are going to do 0-Fold
    for i in range(1):
        ...
        output_dir = config.output_dir / f"{config.arch_type}_cnn" / f"{num_train_samples}" / f"Fold{i}"
        ...
        omr_train_images, amt_train_images, train_labels = omr_train_images_fnames[i], amt_train_images_fnames[i], train_labels_fnames[i]
        omr_val_images, amt_val_images, val_labels = omr_val_images_fnames[i], amt_val_images_fnames[i], val_labels_fnames[i]
        ...
        if num_train_samples is not None and num_val_samples is not None:
            omr_train_images, amt_train_images, train_labels = omr_train_images[:num_train_samples], amt_train_images[:num_train_samples], train_labels[:num_train_samples]
            omr_val_images, amt_val_images, val_labels = omr_val_images[:num_val_samples], amt_val_images[:num_val_samples], val_labels[:num_val_samples]

        assert len(omr_train_images) == len(amt_train_images) == len(train_labels)
        assert len(omr_val_images) == len(amt_val_images) == len(val_labels)
        ...
        test_data = train_and_test_model(
            omr_data=(omr_train_images, omr_val_images, omr_test_images),
            amt_data=(amt_train_images, amt_val_images, amt_test_images),
            gt_data=(train_labels, val_labels, test_labels),
        ...
        # Clear memory
        del omr_train_images, omr_val_images, omr_test_images, amt_train_images, amt_val_images, amt_test_images
