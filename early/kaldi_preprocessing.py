...
from data_processing import load_dictionaries
...
def kaldi_confmat(kaldi_dir, fold_type, id_preds, preds, preds_len):
    with open(kaldi_dir / f"confMat-{fold_type}", "w") as f:
        for id, y_pred, len in zip(id_preds, preds, preds_len):
            ...
            f.write("]\n")
    subprocess.call(["tar", "-czvf", kaldi_dir / f"confMat-{fold_type}.tgz", kaldi_dir / f"confMat-{fold_type}"])
    os.remove(kaldi_dir / f"confMat-{fold_type}")
    return
