...
import config
from data_processing import get_folds_filenames, get_datafolds_filenames, get_fold_vocabularies, save_w2i_dictionary, train_data_generator, load_dictionaries
...
def k_fold_experiment(epochs, num_train_samples, num_val_samples):
    ...
    for i in range(1):
        ...
        output_dir = config.output_dir / f"{num_train_samples}" / config.task / f"Fold{i}"
        ...
        if num_train_samples is not None and num_val_samples is not None:
            ...
            val_images, val_labels = val_images[:num_val_samples], val_labels[:num_val_samples]
